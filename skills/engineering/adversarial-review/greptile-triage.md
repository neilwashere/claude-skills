...
Shared reference for fetching, filtering, and classifying Greptile review comments on GitHub PRs. Both `/adversarial-review` (Step 2.5) and `/ship` (Step 3.75) reference this document.
...
PROJECT_HISTORY="$HOME/.claude/review-history/$REMOTE_SLUG/greptile-history.md"
```
...
Use Tier 2 when escalation detection (below) identifies a prior review-skill reply on the same thread. Include maximum evidence to close the discussion.
...
Before composing a reply, check if a prior review-skill reply already exists on this comment thread:

1. **For line-level comments:** Fetch replies via `gh api repos/$REPO/pulls/$PR_NUMBER/comments/$COMMENT_ID/replies`. Check if any reply body contains review-skill markers: `**Fixed**`, `**Not a bug.**`, `**Already fixed**`.

2. **For top-level comments:** Scan the fetched issue comments for replies posted after the Greptile comment that contain review-skill markers.

3. **If a prior review-skill reply exists AND Greptile posted again on the same file+category:** Use Tier 2 (firm) templates.

4. **If no prior review-skill reply exists:** Use Tier 1 (friendly) templates.
...
```bash
REMOTE_SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.claude/review-history/$REMOTE_SLUG"
...
- `~/.claude/review-history/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.claude/review-history/greptile-history.md` (global aggregate)
