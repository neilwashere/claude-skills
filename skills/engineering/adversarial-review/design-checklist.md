...
**Trigger:** Only run this checklist if the diff touches frontend files. Detect by checking the changed-file list for frontend extensions:

```bash
SCOPE_FRONTEND=false
git diff --name-only origin/<base>...HEAD 2>/dev/null \
  | grep -qiE '\.(tsx|jsx|ts|js|css|scss|vue|svelte|html)$' && SCOPE_FRONTEND=true
