...
#### Completeness Gaps
- Shortcut implementations where the complete version would cost a small amount of additional effort (e.g., partial enum handling, incomplete error paths, missing edge cases that are straightforward to add)
- Options presented without a clear estimated effort — should state the rough effort to complete
...
This heuristic is referenced by both `/adversarial-review` and `/ship`. It determines whether
